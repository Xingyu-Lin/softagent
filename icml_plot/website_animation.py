...
def simulate_policy_RIG(args):
    '''
    For RIG, the wrapper env does not pass arguments in step to wrapped envs. 
    Therefore just hack to change the deterministic idx in flex_env.py, and also set default record extra frames to be true in flex_env.step
    '''
    dir = args.dir
    dirs = os.walk(args.dir)
    subdirs = [x[0] for x in dirs]
    ...
    for i in range(row * column):
        ...
        frames.append(env.get_image(img_size, img_size))
        for _ in range(env.horizon):
            goal = obs['latent_desired_goal']
            obs = obs['latent_observation']
            new_obs = np.hstack((obs, goal))
            action, _ = policy_determinisitic.get_action(new_obs)
    ...
    # Convert to T x index x C x H x W for pytorch
    all_frames = np.array(all_frames).transpose([1, 0, 4, 2, 3])
    grid_imgs = [torchvision.utils.make_grid(torch.from_numpy(frame), nrow=row, padding=20, pad_value=120).permute(1, 2, 0).data.cpu().numpy() for frame in all_frames]

    save_name = args.env + '-' + args.policy + '-' + args.suffix + '.gif'
    save_numpy_as_gif(np.array(grid_imgs), osp.join(save_dir, save_name))
...
if __name__ == "__main__":
    ...
    elif args.policy == 'RIG':
        simulate_policy_RIG(args)
