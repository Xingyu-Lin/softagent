...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x):
        ...
        # the neighborhood matrix is of shape (n_points, max_num_of_neighborhood_per_point)
        # some points will not have max_num_of_neigherhood_per_point such many neighborhoods
        # therefore, a fake idx is used to fill those columns. such neighbor is the shadow neighbor.
        s_pts = torch.cat((s_pts, torch.zeros_like(s_pts[:1, :]) + 1e6), 0)

        # Get neighbor points [n_points, n_neighbors, dim]
        # print(neighb_inds.dtype)
        # print(type(neighb_inds))
...
class BatchNormBlock(nn.Module):
    ...
    def forward(self, x):
        if self.use_bn:
            # print("in blocks batch norm block, input x is: ", x.shape)
            x = x.unsqueeze(2)
            x = x.transpose(0, 2)
            x = self.batch_norm(x)
            x = x.transpose(0, 2)
            # print("in blocks batch norm block, after all process, x is: ", x.shape)
