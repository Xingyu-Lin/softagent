...
def gen_PyFleX(info):
    ...
    if env_idx == 5:    # RiceGrip
        stats = [init_stat(6), init_stat(6)] # NOTE: why rice stats are 6 dims?
    else:
...
def prepare_input(data, stat, args, phases_dict, verbose=0, var=False):
    ...
    if args.env == 'RiceGrip':
        ...
        for i in range(n_shapes):
            ...
            rels += [np.stack([nodes, wall, np.ones(nodes.shape[0])], axis=1)] #NOTE: what does the np.ones mean? Maybe [sender, receiver, value]
            # NOTE: actually the values are just set to one to fill in a sparse matrix tensor.
