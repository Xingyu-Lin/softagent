from DPINet.cem import CEMPolicy
...
def run_task(arg_vv, log_dir, exp_name):
    ...
    policy = CEMPolicy(env, env_class, env_kwargs, vv['use_mpc'], plan_horizon=vv['plan_horizon'], max_iters=vv['max_iters'],
                       population_size=vv['population_size'], num_elites=vv['num_elites'], model_path=vv['model_path'])
    # Run policy
    ...
    for i in range(vv['test_episodes']):
        ...
        clusters = []
        kmeans = MiniBatchKMeans(n_clusters=30, random_state=0).fit(pos)
        clusters.append([[kmeans.labels_]])
        policy.reset(clusters)
