...
def get_snapshot_specified():
    return _snapshot_specified


def set_snapshot_specified(specified):
    global _snapshot_specified
    _snapshot_specified = specified
...
def save_itr_params(itr, params, use_cloudpickle=True, pkl_prefix=''):
    if _snapshot_dir:
        if _snapshot_mode == 'all':
            ...
            return
        elif _snapshot_mode == 'specified':
            if str(itr) in _snapshot_specified:
                file_name = osp.join(get_snapshot_dir(), pkl_prefix + 'itr_%d' % itr)
                from pathlib import Path
                file_name_path = Path(file_name + '.pkl')
