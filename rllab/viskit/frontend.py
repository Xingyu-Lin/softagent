...
def make_plot(plot_list, use_median=False, use_five_numbers=False, plot_width=None, plot_height=None, title=None,
              xlim=None, ylim=None, legendNote=None):
    data = []
    p25, p50, p75 = [], [], []
    p0, p100 = [], []
    ...
    elif legendNote is not None:
        footnote = "<br />".join([
            r"<span><b>%s</b></span>: <span>%s</span>" % ('Legend Note', legendNote)
        ])
        return r"%s<div>%s</div>" % (fig_div, footnote)
    else:
        return fig_div
  ...
  legendNote=None,
    ...
    for split_selector, split_legend in zip(split_selectors, split_legends):
        if custom_series_splitter is not None:
            ...
            splitted_dict = OrderedDict(sorted(splitted_dict.items(), key=lambda t: t[0]))
...
def plot_div():
    ...
    preset_name_index = int(args.get("preset_name_index", "0"))
    ...
    legendNote = None
    if preset_name_index != 0:
        preset_file = os.path.join(g_datapath[0], 'presets.py')
        import importlib.util
        spec = importlib.util.spec_from_file_location('presets', preset_file)
        presets = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(presets)
        preset_names = presets.preset_names
        print('preset_name', preset_names[preset_name_index - 1])

        custom_series_splitter, legendNote = presets.make_custom_seris_splitter(preset_names[preset_name_index - 1])
        if hasattr(presets, 'make_custom_filter'):
            print('Found custom filter')
            custom_filter = presets.make_custom_filter(preset_names[preset_name_index - 1])
...
def index():
    ...
    if "test/success_rate" in plottable_keys:
        plot_key = "test/success_rate"
    ...
    # TODO add preset names here
    preset_file = os.path.join(g_datapath[0], 'presets.py')
    if os.path.isfile(preset_file):
        print('Load presets from ', preset_file)
        import importlib.util
        spec = importlib.util.spec_from_file_location('presets', preset_file)
        presets = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(presets)
        ...
        print('No presets file ', preset_file)
        preset_names = []
