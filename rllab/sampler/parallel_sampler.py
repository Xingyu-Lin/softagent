...
def _worker_terminate_task(G, scope=None):
    if hasattr(G, 'graph'):
        G.graph.__exit__(None, None, None)
    if hasattr(G, 'sess'):
        G.sess.__exit__(None, None, None)
    G = _get_scoped_G(G, scope)
...
def _worker_set_policy_params(G, params, scope=None):
    G = _get_scoped_G(G, scope)
    if hasattr(G.policy, 'get_param_values_with_baseline'):
        G.policy.set_param_values_with_baseline(params)
    else:
