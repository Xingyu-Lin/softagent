...
def skewfit_full_experiment(variant, log_dir, exp_prefix):
    ...
    variant = variant['skewfit_kwargs']
...
def full_experiment_variant_preprocess(variant):
    ...
    if 'env_id' in variant:
        ...
        train_vae_variant['generate_vae_dataset_kwargs']['env_kwargs'] = variant['env_arg_dict']
    else:
    ...
    if dataset_path is not None:
        ...
        if use_cached and osp.isfile(filename):
            ...
            if env_id is not None:
                ...
                if env_id in SOFTGYM_CUSTOM_ENVS.keys():
            ...
            for i in range(N):
                if random_and_oracle_policy_data:
                    ...
                    print("use random actions to get initial vae training data, epoch {}".format(i))
                    env.reset()
                    for _ in range(n_random_steps):
                        obs = env.step(env.action_space.sample())[0]
