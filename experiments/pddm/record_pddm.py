...
def pddm_make_gif(env, action_trajs, save_dir, save_name, img_size=128, config_ids=None):
    all_frames = []
    for i in range(len(action_trajs)):
        frames = []
        if config_ids is not None:
            env.reset(config_id=config_ids[i])
        else:
            env.reset()
...
def generate_video(file_paths):
    ...
    for env, env_name, file_path in zip(envs, env_names, file_paths):
        ...
        pddm_make_gif(env, initial_states, action_trajs, configs, save_dir='data/videos/cem', save_name=env_name + '.gif')
